class JobSeries < ActiveRecord::Base
attr_accessor :title, :description, :commit_button
  ...
  after_create :create_jobs_until_end_time
  
  def create_jobs_until_end_time(end_time=END_TIME)
    st = starttime
    et = endtime
    p = r_period(period)
    nst, net = st, et
...
#      puts "#{nst}           :::::::::          #{net}" if nst and net
      self.jobs.create(:title => title, :description => description, :all_day => all_day, :starttime => nst, :endtime => net)
